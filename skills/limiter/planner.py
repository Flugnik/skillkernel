...
def build_export_plan(event_id: str, delivery_date: date) -> ActionPlan:
    """Build an ActionPlan that creates an xlsx export for the given date.

    Only serialisable data is stored in Action.params so that
    ActionPlan / SkillResult can be safely serialised via model_dump(mode="json").
    The actual xlsx bytes are generated by the executor at execution time.
    """
    from skills.limiter.exporter import export_path

    path = export_path(delivery_date)
    preview_text = (
        f"Будет создан Excel-экспорт по дате {delivery_date.isoformat()}.\n"
        f"Файл: {path}"
    )
